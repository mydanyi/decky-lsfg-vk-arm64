...
class ConfigurationService(BaseService):
    ...
    def update_lsfg_script(self, config: ConfigurationData) -> ConfigurationResponse:
        """Update the ~/lsfg-arm64 launch script with current configuration
    ...
    def _generate_script_content(self, config: ConfigurationData) -> str:
        """Generate the content for the ~/lsfg-arm64 launch script
        ...
        lines = [
            "#!/bin/bash",
            "# lsfg-vk launch script generated by LSFG-VK ARM64 plugin",
    ...
    def _generate_script_content_for_profile(self, profile_data: ProfileData) -> str:
        """Generate the content for the ~/lsfg-arm64 launch script with profile support
    ...
    def _runtime_lines(self, config: dict, profile: str) -> list[str]:
        if not self._uses_v2():
            # Only the owned ARM64 v2 runtime is supported: never fall back to
            # the original engine's LSFG_PROCESS launcher or another plugin.
            raise RuntimeError(
                "Owned ARM64 v2 layer manifest is missing or obsolete; "
                "install the runtime before generating a launcher")
    ...
    def update_lsfg_script_from_profile_data(self, profile_data: ProfileData) -> ConfigurationResponse:
        """Update the ~/lsfg-arm64 launch script from profile data
